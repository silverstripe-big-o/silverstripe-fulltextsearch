...
abstract class SolrIndex extends SearchIndex {
	...
	public function search(SearchQuery $query, $offset = -1, $limit = -1, $extraParams = array()) {
		...
		if(!headers_sent()) {
			if ($q) header('X-Query: '.$q);
			if (isset($params['fq'])) header('X-Filters: "'.$params['fq'].'"');
		}
	...
	function getQueryParams($query, $limit = -1, $offset = -1, $extraParams = array()) {
		SearchVariant::with(count($query->classes) == 1 ? $query->classes[0]['class'] : null)->call('alterQuery', $query, $this);
		$q = array();
		$fq = array();
		$queryStopWords = array('AND', 'NOT', 'OR');
		...
		foreach ($query->search as $search) {
			...
			// Split by quoted phrases
			preg_match_all('/"[^"]*"|\S+/', $text, $parts);

			$fuzzy = $search['fuzzy'] ? '~' : '';
			$isBoolean = (array_intersect($parts[0], $queryStopWords));

			// Treat each phrase separately
			foreach ($parts[0] as $part) {
				...
				elseif($isBoolean || preg_match('/^-/', $part)) {
					// Don't mark values as required if its a boolean query
					$q[] = $fuzzy.$part;
				} else {
					// Mark values as required by default
					$q[] = '+'.$fuzzy.$part; 
				}
			}
		}

		// Fall back to alias filter for everything
		if(!$q) $q = array('*:*');
	...
	public function getService() {
		if(!$this->service) $this->service = Solr::service($this->getName());
		return $this->service;
	}
